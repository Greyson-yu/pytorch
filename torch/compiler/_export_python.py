...
def _dedupe_kernels(code: str) -> str:
    """Drop kernel blocks that a later identical block shadows.

    Inductor can emit the same NAME = async_compile.triton(NAME, ...) block more than
    once; the last assignment wins and the earlier ones are dead. That is invisible in a
    file the reader is invited to tune: they find the first copy, change a block size,
    rerun, and nothing happens. Only byte-identical duplicates are removed, so this
    cannot change what the artifact computes.
    """
    terminator = "'''" + ", device_str="
    lines = code.splitlines(True)
    blocks: dict[str, list[tuple[int, int, str]]] = {}
    i = 0
    while i < len(lines):
        match = re.match(r"(\w+) = async_compile\.triton\(", lines[i])
        if match:
            for j in range(i + 1, len(lines)):
                if lines[j].startswith(terminator):
                    blocks.setdefault(match.group(1), []).append(
                        (i, j + 1, "".join(lines[i : j + 1]))
                    )
                    i = j
                    break
        i += 1
    ...
    for spans in blocks.values():
        if len(spans) < 2:
            continue
        last = spans[-1][2]
        drop.extend((a, b) for a, b, body in spans[:-1] if body == last)
    if not drop:
        return code
    for a, b in sorted(drop, reverse=True):
        del lines[a:b]
    deduped = "".join(lines)
    # Earn the claim above rather than asserting it. This is a text transform over
    # generated source, and the block scan can in principle mis-terminate on a kernel
    # whose own body contains the terminator, so the result has to still parse and still
    # bind exactly the same module-level names. If it does not, publish the original: a
    # duplicate kernel is a hill-climbing annoyance, a corrupted artifact is not.
    try:
        if _module_level_names(deduped) != _module_level_names(code):
            return code
    except SyntaxError:
        return code
    return deduped
...
def _check_tolerances(dtype: torch.dtype) -> tuple[float, float]:
    """Per-dtype defaults, overridable by env var.

    A fixed 1e-5 rejects an honest bf16 or fp16 artifact, because inductor's fusion and
    accumulation order differ from eager's well beyond that at reduced precision -- and
    the error would tell the user to recapture, producing an artifact that fails the
    same way.
    """
    ...
    return (
        float(override_rtol) if override_rtol else max(rtol, 1e-5),
        float(override_atol) if override_atol else max(atol, 1e-5),
    )
